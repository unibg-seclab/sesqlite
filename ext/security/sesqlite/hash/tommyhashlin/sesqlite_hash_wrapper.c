...
static void freeElement(
	...
	tommy_hashlin_remove_existing(th->hash, &elem->node);
...
void tommyHashInit(
	...
	th->hash = malloc(sizeof(tommy_hashlin));
	tommy_hashlin_init(th->hash);
}
...
void tommyHashInsert(
	...
	tommy_hashlin_insert(th->hash, &obj->node, obj, HASHCODE(pKey, nKey));
}
...
void tommyHashFind(
	...
	tommyElement *obj = tommy_hashlin_search(th->hash, compareElement, &tp, HASHCODE(pKey, nKey));
...
void tommyHashClear(
	...
	tommy_hashlin_foreach_arg(th->hash, freeElement, th);
}
