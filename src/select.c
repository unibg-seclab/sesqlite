...
fprintf(stdout, "scon: %s, tcon: %s => %d\n", 
	...
	Expr *pFDebug = sqlite3DbMallocZero(db, sizeof(Expr) + strlen(pSrc->a[j].zName) + 1);
	...
	memcpy(pFDebug->u.zToken, zName, strlen(pSrc->a[j].zName));
	...
	pFDebug->u.zToken[strlen(pSrc->a[j].zName)] = 0;
